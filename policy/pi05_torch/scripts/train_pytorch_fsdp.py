...
import tqdm
import wandb
from pi.training.config import TrainConfig, pi_config, LeRobotAirbotDataConfig, AssetsConfig, _optimizer
...
def main(argv: List[str]) -> int:
    ...
    if args.data_root:
        ...
        cfgs = [
            dataclasses.replace(
                config0, data=dataclasses.replace(config0.data, repo_id="/data/robot/0922_250samples_merge")
            ),
            dataclasses.replace(
                config0, data=dataclasses.replace(config0.data, repo_id="/data/robot/0925_401samples_merge")
            ),
        ]
