...
import pi.training.data_loader as _data
...
def train_loop(config: _config.TrainConfig):
    ...
    # Build model
    if not isinstance(config.model, pi.models.pi_config.PiConfig):
        # Convert dataclass to Pi0Config if needed
        model_cfg = pi.models.pi_config.PiConfig(
    ...
    model = pi.models_pytorch.pi0_pytorch.PI0Pytorch(model_cfg).to(device)
    ...
    while global_step < config.num_train_steps:
        ...
        for observation, actions in loader:
            ...
            # Move observation tensors to device (PyTorch equivalent of jax.tree.map)

            def _tree_map_to_device(item, target_device):
                """Recursively move tree structures to target device."""
                if isinstance(item, dict):
                    return {key: _tree_map_to_device(value, target_device) for key, value in item.items()}
                elif isinstance(item, (list, tuple)):
                    result = [_tree_map_to_device(value, target_device) for value in item]
                    ...
                    if dataclasses.is_dataclass(item):
                        ...
                        for field in dataclasses.fields(item):
                            ...
                            new_attrs[field.name] = _tree_map_to_device(attr_value, target_device)
                        ...
                        for attr_name, attr_value in item.__dict__.items():
                            new_attrs[attr_name] = _tree_map_to_device(attr_value, target_device)
            ...
            observation = _tree_map_to_device(observation, device)
