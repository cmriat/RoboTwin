...
from transformers.utils import ModelOutput, logging, torch_int, auto_docstring, can_return_tuple
...
class SiglipVisionModelOutput(ModelOutput):
    r"""image_embeds (`torch.FloatTensor` of shape `(batch_size, output_dim)` *optional* returned when model is initialized with `with_projection=True`).
...
class SiglipTextModelOutput(ModelOutput):
    r"""text_embeds (`torch.FloatTensor` of shape `(batch_size, output_dim)` *optional* returned when model is initialized with `with_projection=True`).
...
class SiglipOutput(ModelOutput):
    r"""loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `return_loss` is `True`).
...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: torch.Tensor, height: int, width: int) -> torch.Tensor:
        ...
        This method is also adapted to support torch.jit tracing and no class embeddings.
